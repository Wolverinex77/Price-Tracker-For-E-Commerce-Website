...
def handle_request(url):
    ...
    USE_PROXY = False   #Change to True to Turn on Proxy
    try:
        if USE_PROXY:
...
def scrape_category_product_data(response,url):
    ...
    for product in products:
        ...
        price=price_tag.getText(strip=True) if price_tag else "No Price found"
        link=product.a["href"]
        
        #Discounts -> Email()
        retail_tag=product.find("div",class_="price-diff-retail")
        retail_price=retail_tag.find("span").get_text(strip=True) if retail_tag else price
            
        discount_tag=product.find("div",class_="price-diff-saving")
